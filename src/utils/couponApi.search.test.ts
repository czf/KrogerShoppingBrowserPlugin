...
describe('fetchCoupons — searchString param', () => {
  beforeEach(() => {
  ...
  it('includes filter.searchString in request URL', async () => {
    ...
    await fetchCoupons({ pageSize: 24, searchString: 'banana' });
    expect(fetch).toHaveBeenCalled();
    const calledUrl = (vi.mocked(fetch).mock.calls[0][0]) as string;
    expect(calledUrl).toContain('filter.searchString=banana');
  });
});
